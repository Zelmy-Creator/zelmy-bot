...
# --- 2. КОНФИГУРАЦИЯ ---
BOT_TOKEN = os.environ.get("BOT_TOKEN", "ТВОЙ_TOKEN")
...
def track_user(user):
    ...
    users_db[str_id] = {
        "username": user.username or "нет_измерения",
...
def handle_referral(user_id, ref_id):
    ...
    if user_id == ref_id:
        return
    if users_db.get(user_id, {}).get("referred_by"):
        return  # уже привязан
    if ref_id not in users_db:
        return
    users_db[user_id]["referred_by"] = ref_id
    users_db[ref_id]["referral_count"] = users_db[ref_id].get("referral_count", 0) + 1
    save_json(USERS_FILE, users_db)

    count = users_db[ref_id]["referral_count"]
    if count % REFERRAL_BONUS_EVERY == 0:
        ...
        save_json(SUBSCRIPTIONS_FILE, subscriptions)
        try:
            bot.send_message(int(ref_uid), f"🎉 Ты пригласил уже {count} друзей! Начислено +{REFERRAL_BONUS_DAYS} дня Premium.")
        except Exception as e:
...
def is_premium(user_id):
    if is_admin(user_id):  # создатель считается премиумом везде, где это нужно (например /profile)
        return True
    user_id = str(user_id)
...
def get_subscription_reminder(user_id):
    ...
    if days_left < 0:
        return "⚠️ Подписка истекла. Продли: /premium"
    if days_left <= 1:
        return "⏳ Подписка заканчивается сегодня! Продли: /premium"
    if days_left <= 3:
...
# --- 6. ПОИСК ---
def _dedupe_by_domain(results, max_results):
    seen = set()
    unique = []
    for r in results:
        if not r.get('link'):
...
def search_web(query, max_results=5, region='ru-ru'):
    try:
        ddgs = DDGS()
        results = []
        kwargs = {'max_results': max_results * 2, 'region': region}
    ...
    try:
        ...
        response = requests.get(url, params={'q': query}, timeout=25, headers=headers)
...
def stream_groq_reply(chat_id, payload, original_message=None, existing_message_id=None):
    """Отправляет плейсхолдер (или переиспользует existing_message_id при повторной попытке)"""
    try:
        ...
        logging.error(f"Не удалось создать плейсхолдер: {e}")
        return None, None
            ...
            for raw_line in response.iter_lines():
                ...
                if now - last_edit_time >= STREAM_EDIT_INTERVAL and full_text != last_shown:
                    try:
                        ...
                        pass  # "message is not modified" и подобные – не критично, просто пропускаем кадр
    except Exception as e:
    ...
    if not full_text.strip():
        return None, placeholder_id
    return full_text, placeholder_id
...
def process_llm_request(chat_id, user_id, text, original_message=None):
    ...
    try:
        ...
        # --- ФИЛЬТР ЭКСТРЕМИЗМА (до любой обработки, включая поиск и LLM) ---
        ...
        # Фильтр safe=true у Pollinations не гарантирует безопасный контент.
        # Включать обратно только после того, как появится проверенная модерация вывода.
            ...
            if search_results:
                ...
                try:
                    ...
                    if fallback.status_code == 200:
                        answer = fallback.json()['choices'][0]['message']['content']
                        plain_reply(answer)
                    else:
                        plain_reply("❌ Поиск не дал результатов, и Groq не смог ответить.")
                except Exception as e:
                    plain_reply("❌ Поиск не дал результатов.")
                return

        # --- ОБЫЧНЫЙ ДИАЛОГ ---
        memory_limit = 50 if has_plan_at_least(user_id, "premium") else 10
        ...
        sys_prompt = {"role": "system", "content": (
            ...
            "жирный заголовок в HTML: <b>Заголовок</b> перед абзацем. Не пиши одной сплошной стеной текста. "
            "Короткие ответы (факт, да/нет, короткое пояснение) оставляй просто короткими, без искусственного деления.\n"
        ...
        out, placeholder_id = stream_groq_reply(
            chat_id,
            {"model": CURRENT_MODEL, "messages": payload, "temperature": 0.5, "max_tokens": 2000},
            original_message
        )
        if not out:
            # одна повторная попытка в то же самое сообщение, если генерация споткнулась (сеть/лимиты API)
            out, placeholder_id = stream_groq_reply(
                chat_id,
                {"model": CURRENT_MODEL, "messages": payload, "temperature": 0.5, "max_tokens": 2000},
                original_message,
                existing_message_id=placeholder_id
            )
        if out:
...
def check_sub_callback(call):
    try:
        ...
        if member.status in ['creator', 'administrator', 'member']:
            sub_cache[call.from_user.id] = (True, time.time())
...
def image_cmd(message):
    # Генерация временно отключена: фильтр safe=true у Pollinations
    # не блокирует откровенный контент, а pytesseract/бот не могут
    # достоверно проверять результат перед отправкой пользователю.
...
def stats_cmd(message):
    ...
    text = (
        ...
        f"📋 Последних ошибок в логе: {len(stats_db['errors'])}\n"
...
def text_handler(message):
    ...
    if text in BUTTON_ACTIONS:
        ...
        return
    if text == "Поиск":
        bot.reply_to(message, "Напиши запрос после /search, например:\n<code>/search курс доллара</code>", parse_mode="HTML")
        return
    if text == "Картинка":
        bot.reply_to(message, "Напиши описание после /image, например:\n<code>/image кота в очках</code>", parse_mode="HTML")
        return
    if text == "Фото":
...
# --- 13. ЗАПУСК БОТА ---
# ВАЖНО про "асинхронность": pyTelegramBotAPI уже обрабатывает каждое сообщение в отдельном
# потоке при threaded=True (это явно включено ниже) – долгий запрос одного пользователя
