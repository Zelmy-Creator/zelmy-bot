...
при двух одновременных записях (два юзера пишут боту в одну секунду) можно
было потерять данные (последняя запись просто перезатирала файл без учёта
параллельной). SQLite с блокировкой на запись решает это, плюс он ощутимо
быстрее на больших объёмах (не нужно перечитывать/переписывать весь файл
ради одной строки).
...
def _migrate_from_json_if_needed():
    ...
    if usage:
        for uid, info in usage.items():
            with _lock:
                _conn.execute(
                    "INSERT OR REPLACE INTO usage_daily (user_id, date, count) VALUES (?, ?, ?)",
    ...
    if promo:
        for code, info in promo.items():
            with _lock:
                _conn.execute(
                    "INSERT OR REPLACE INTO promocodes (code, plan, days, max_uses, created_by) VALUES (?, ?, ?, ?, ?)",
...
def register_user(user_id, username, first_name):
    ...
    with _lock:
        ...
        if row is None:
            _conn.execute("""
                INSERT INTO users (id, username, first_name, first_seen, banned, referred_by, referral_count, persona)
...
def count_active_subscriptions():
    row = _conn.execute("SELECT COUNT(*) c FROM subscriptions WHERE expires_at > ?", (time.time(),)).fetchone()
    return row["c"]
...
def get_history_messages(chat_id, limit=50):
    rows = _conn.execute(
...
def create_promocode(code, plan, days, max_uses, created_by):
    with _lock:
        _conn.execute(
            "INSERT INTO promocodes (code, plan, days, max_uses, created_by) VALUES (?, ?, ?, ?, ?)",
...
def track_daily_active(user_id):
    ...
    with _lock:
        _conn.execute(
            "INSERT OR IGNORE INTO stats_daily_active (date, user_id) VALUES (?, ?)",
            (today, user_id)
        )
        _conn.commit()
...
def track_error(context, error_text):
    with _lock:
        _conn.execute(
            "INSERT INTO stats_errors (time, context, error) VALUES (?, ?, ?)",
            (datetime.now().strftime("%Y-%m-%d %H:%M:%S"), context, str(error_text)[:300])
        )
        # держим только последние 200 записей
        _conn.execute("""
            DELETE FROM stats_errors
            WHERE id NOT IN (SELECT id FROM stats_errors ORDER BY id DESC LIMIT 200)
        """)
        _conn.commit()
...
def start_campaign(plan, days, slots):
    with _lock:
        ...
        _conn.execute(
            "INSERT INTO campaign (id, plan, days, slots_left) VALUES (1, ?, ?, ?)",
            (plan, days, slots)
        )
        _conn.commit()
